"""
Hermes Bridge for Synthesis MCP Tools

This module bridges Synthesis's FastMCP tools with Hermes MCP aggregator,
allowing Synthesis's tools to be discoverable and executable through Hermes.
"""
...
class SynthesisMCPBridge(MCPService):
    """
    Bridge between Synthesis's FastMCP tools and Hermes MCP aggregator.
    
    This class allows Synthesis to register its tools with Hermes while
    maintaining the existing FastMCP implementation.
    """
    
    def __init__(self, synthesis_engine=None, component_name: str = "synthesis"):
        """Initialize the Synthesis MCP Bridge."""
    ...
    async def register_default_tools(self):
        ...
        info_tool = ComponentInfoTool(
            ...
            component_description="Data synthesis and integration orchestration engine"
        )
    ...
    async def register_fastmcp_tool(self, fastmcp_tool: Dict[str, Any]):
        ...
        async def handler(parameters: Dict[str, Any]) -> Dict[str, Any]:
            ...
            from synthesis.api.fastmcp_endpoints import fastmcp_server
            ...
            try:
                ...
                tool_func = fastmcp_tool.get('function')
                if tool_func:
                    result = await tool_func(**parameters)
                    return result
                else:
                    raise Exception(f"Tool {fastmcp_tool['name']} has no function")
            except Exception as e:
        ...
        if self.hermes_client:
            try:
                ...
                await self.hermes_client.register_tool(
                    ...
                    metadata={
                        'category': 'synthesis',
                        'tags': ['integration', 'synthesis', 'orchestration'],
                        'fastmcp': True
                    }
                )
    ...
    async def _get_health_status(self) -> Dict[str, Any]:
        """Get health status from Synthesis engine."""
        try:
            # Basic health check
            components_status = {
                "data_synthesis": True,
            ...
            all_healthy = all(v for k, v in components_status.items() if k != "tools_loaded")
